...
contract Sender_Auth {
    ...
    function verify_email(string memory _email) public returns (bool valid) {
        uint256 len = bytes(_email).length;
        if (len < 10) {
            return false;
        } else {
            return true;
    ...
    function verify_psd(string memory _psd, string memory _psd_hint)
        public
        view
        returns (bool valid)
    {
        uint256 len1 = bytes(_psd).length;
        uint256 len2 = bytes(_psd_hint).length;
